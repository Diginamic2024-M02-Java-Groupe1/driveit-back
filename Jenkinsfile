      ...
      steps {
        script {
          ...
          withSonarQubeEnv('SonarQ') { // 'SonarQ' est le nim de la connexion SonarQube dans Jenkins
            sh "${mvnHome}/bin/mvn clean verify sonar:sonar -Dsonar.projectKey=Vulnado -Dsonar.projectName='Vulnado'" // -Dsonar.projectKey & -Dsonar.projectName doivent correspondre au nom du projet créé dans SonarQube
          }
        }
      }
    }
